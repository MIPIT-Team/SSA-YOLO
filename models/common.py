...
class Conv(nn.Module):
    ...
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, d=1, act=True):
        super().__init__()
        self.conv = nn.Conv2d(c1, c2, k, s, autopad(k, p, d), groups=g, dilation=d, bias=False)  # Conv2d：pytorch中2维卷积
...
class ASFFV5(nn.Module):
    def __init__(self, level, multiplier=0.5, rfb=False, vis=False, act_cfg=True):
        ...
        self.dim = [int(512 * multiplier), int(256 * multiplier),
                    int(128 * multiplier)] # 408 312 168
        # print(self.dim)

        self.inter_dim = self.dim[self.level]
        if level == 0:
            self.stride_level_1 = Conv(int(256 * multiplier), self.inter_dim, 3, 2)#312

            self.stride_level_2 = Conv(int(128 * multiplier), self.inter_dim, 3, 2)#168

            self.expand = Conv(self.inter_dim, int(512 * multiplier), 3, 1)#408
        elif level == 1:
            self.compress_level_0 = Conv(
                int(512 * multiplier), self.inter_dim, 1, 1)#408 168 312
            self.stride_level_2 = Conv(
                int(128 * multiplier), self.inter_dim, 3, 2)
            self.expand = Conv(self.inter_dim, int(256 * multiplier), 3, 1)
        elif level == 2:
            self.compress_level_0 = Conv(
                int(512 * multiplier), self.inter_dim, 1, 1)#408 312 168
...
class ASFF_Detect(nn.Module):  # add ASFFV5 layer and Rfb
    ...
    def __init__(self, nc=80, anchors=(), ch=(), multiplier=1, rfb=False, inplace=True):  # detection layer
        super().__init__()
    ...
    def forward(self, result):  # Change 'x' to 'result'
        z = []  # inference output
        for i in range(self.nl):
            result[i] = self.m[i](result[i])  # conv
            bs, _, ny, nx = result[i].shape  # x(bs,255,20,20) to x(bs,3,20,20,85)
            result[i] = result[i].view(bs, self.na, self.no, ny, nx).permute(0, 1, 3, 4, 2).contiguous()

            if not self.training:  # inference
                if self.onnx_dynamic or self.grid[i].shape[2:4] != result[i].shape[2:4]:
        ...
        return result if self.training else (torch.cat(z, 1),) if self.export else (torch.cat(z, 1), result)
...
class C3SimAM(C3):
    # C3 module with SimAM()
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):
        super().__init__(c1, c2, n, shortcut, g, e)
        c_ = int(c2 * e)  # hidden channels
        self.m = nn.Sequential(*(SimAM(c_, c_,shortcut) for _ in range(n)))
